...
public abstract class AbstractRestClient<T extends IDto> implements IRestClient<T> {
    ...
    final String createAsUri(final T resource, final Pair<String, String> credentials) {
        ...
        final String locationOfCreatedResource = response.jsonPath().getString("_links.self.href");
    ...
    public final void delete(String uri) {
        final Response deleteResponse = deleteAsResponse(uri);
        Preconditions.checkState(deleteResponse.getStatusCode() == 204);
    }

    @Override
    public final Response deleteAsResponse(String uri) {
        return givenDeleteAuthenticated().delete(uri);
    }
